...
Try it: `npm run playground` builds and opens
...
`repeatStructures: true` to draw it everywhere; the far side is then flipped if
`structure.mirror` allows it, and the flip is applied so that atom labels are
never mirror-written: each glyph is turned back about its own anchor and the
glyphs of one label are re-laid so `NH₂` still reads `NH₂`.
...
payload: {
  ...
  structure: {
    ...
    attach: { x: 1, y: 0.5 },             // where the bond meets it, as fractions
...
lands exactly on the end of the bond leaving the domain, and it is flipped
horizontally if that atom would otherwise point away from the antibody (set
`mirror: false` for a depiction with stereo wedges). It stays upright however far
its domain is tilted, and the diagram's viewBox widens to fit. Crop your artwork
tight: a viewBox with a wide margin puts empty space where the bond should meet
the molecule — `scripts/adc-demo.mjs` shows how to compute a tight one from a
toolkit's output.
...
(they stay upright whatever angle their domain sits at), and note that the linear
view labels its bars regardless.
...
a DAR the chemistry cannot reach. Each finding carries `refs` in the form
`highlight` takes, so a UI can light up exactly what the message is about.

Rules that read something other than the structure are marked `heuristic` in
`LINT_RULES` and can be turned off: `lint(construct, { disable: ['effector-active-engager'] })`.
