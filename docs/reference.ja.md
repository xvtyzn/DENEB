...
`repeatStructures: true` を指定してください。構造式は既定では反転しません。反対側の
コピーを反転してよい場合だけ `structure.mirror: true` を指定してください。反転時も
原子ラベルは読める向きに保たれます。
...
先端にちょうど乗るように配置されます。`viewBox` がある場合は同じ座標系で指定し、
ない場合は描画枠に対する比率で指定します。構造式は既定では反転しません。くさび結合
などの立体情報を反転してよい場合だけ `mirror: true` を指定すると、必要に応じて
左右反転します。
